...
module.exports = {
	...
	async execute(interaction) {
        ...
        if (interaction.guild.members.me.roles.highest.position <= role.position) return interaction.reply(new ContainerMessage(messages.errors.BOT_ROLE_TOO_LOW.replaceAll('{botRole}', `<@&${interaction.guild.members.me.roles.cache.find(role => role.tags?.botId === botMember.id)}>`).replaceAll('{role}', `<@&${role.id}>`)).isEphemeral().build());
